...
import { MapPin, Clock, IndianRupee, Train, Calendar, Star, ArrowRight, Edit2, Trash2, Shield } from 'lucide-react';
...
const TrainsPage = () => {
  ...
  const fetchTrains = async () => {
    ...
    try {
      ...
      const res = await axios.get('http://localhost:5000/api/trains', { params });
      setTrains(res.data);
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      await axios.delete(`http://localhost:5000/api/admin/service/train/${id}`, config);
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Available Trains</h1>
            <p className="text-sm text-gray-500 mt-1">
              {loading ? 'Loading...' : `${trains.length} trains found`}
            </p>
          </div>
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-xl p-6 mb-6 text-center">
            ...
            <button onClick={fetchTrains} className="mt-3 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700">Retry</button>
          </div>
        )}
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
                <p className="text-gray-500 text-lg">No trains found matching your criteria.</p>
              </div>
            ) : (
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    ...
                    <div className="flex items-center space-x-4 lg:w-1/4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-1 mt-1">
                          ...
                          <span className="text-xs text-gray-600">{train.rating || 'N/A'}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-8 lg:w-2/4">
                      <div className="text-center">
                        ...
                        <p className="text-sm text-gray-600">{train.source}</p>
                      </div>
                    ...
                    <div className="lg:w-1/4 flex items-center justify-between lg:justify-end gap-6">
                      <div className="text-right">
                        <p className="text-xs text-gray-500 mb-1">{train.class} Fare</p>
                        <div className="flex items-center justify-end space-x-1">
                          <IndianRupee className="h-4 w-4 text-gray-400" />
                          <span className="text-2xl font-bold text-gray-900">{train.price?.toLocaleString('en-IN') || '0'}</span>
                        </div>
                      </div>
                      <button
                        onClick={() => handleBooking(navigate, 'train', train, { passengers })}
      ...
      <EditServiceModal
        isOpen={showModal}
        onClose={() => { setShowModal(false); setEditingTrain(null); }}
        serviceType="train"
        item={editingTrain}
        onSave={fetchTrains}
      />
