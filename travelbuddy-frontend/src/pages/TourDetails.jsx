...
import { MapPin, Star, CheckCircle, Shield, Clock, Users } from 'lucide-react';
...
const TourDetails = () => {
  ...
  useEffect(() => {
    if (!tour) {
      toast.error('Tour information not found');
      navigate('/tours');
    }
  }, [tour, navigate]);
  ...
  const highlights = tour.highlights || ['Sightseeing', 'Guide', 'Meals', 'Transport'];
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">{tour.title || 'Tour Name'}</h1>
              <div className="flex items-center gap-4 text-sm text-gray-600">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-semibold text-gray-900">{tour.rating || 'N/A'}</span>
                  <span className="text-gray-400">({tour.reviews || 0} reviews)</span>
                </div>
              </div>
            </div>
            <div className="text-left md:text-right">
              <p className="text-sm text-gray-500 mb-1">Tour Type</p>
              <p className="text-lg font-bold text-gray-900">{tour.type || 'Tour'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="flex items-center gap-3 mb-4 p-4 bg-blue-50 rounded-xl border border-blue-100">
                ...
                <div>
                  ...
                  <p className="font-bold text-gray-900">{tour.duration || 'Not specified'}</p>
                </div>
              </div>

              <h3 className="font-bold text-gray-900 mb-3">About This Tour</h3>
              <p className="text-gray-600 leading-relaxed mb-2">
                Experience an unforgettable journey to <strong>{tour.location}</strong> with this carefully curated {tour.type?.toLowerCase()} tour.
              </p>
              <p className="text-gray-600 leading-relaxed">
                Top highlights include {highlights.slice(0, 3).join(', ')} and many more exciting activities.
              </p>
            </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-6">
              <h3 className="text-lg font-bold text-gray-900 mb-4">Your Tour</h3>
              ...
              <button
                ...
                className="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl transition-colors shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40"
              >
                Book Now
              </button>
