...
import {
  ...
  Star, ArrowRight, Filter, Compass, Globe, Shield, Headphones,
  Heart, Award, Edit2, Trash2
} from 'lucide-react';
...
const SearchResults = () => {
  ...
  // Passenger count state (sync with URL and TopSearchBar)
  const [passengers, setPassengers] = useState(() => {
    const urlPassengers = parseInt(searchParams.get('passengers'));
    return urlPassengers && urlPassengers > 0 ? urlPassengers : 1;
  });

  // Handler for passenger count updates from TopSearchBar
  const handleTravellersChange = (newCount) => {
  ...
  const sortedTrains = useMemo(() => {
    ...
    if (trainSortBy === 'price') {
      ...
      sorted.sort((a, b) => {
        const aDur = a.duration || '0h 0m';
        const bDur = b.duration || '0h 0m';
        const aDuration = parseInt(aDur.replace('h', '').replace('m', '').split(' ')[0]) * 60 + parseInt(aDur.match(/(\d+)m/)?.[1] || 0);
        const bDuration = parseInt(bDur.replace('h', '').replace('m', '').split(' ')[0]) * 60 + parseInt(bDur.match(/(\d+)m/)?.[1] || 0);
  ...
  const handleEdit = (item, serviceType) => {
    setEditingItem(item);
    setShowModal(true);
    // Store service type in a ref or state; using a simple approach by setting editingType separately
  };
  ...
  const handleDelete = async (id, serviceType) => {
    ...
    try {
      ...
      // Refresh data
      if (serviceType === 'flight') fetchFlights();
      else if (serviceType === 'train') fetchTrains();
      else if (serviceType === 'cab') fetchCabs();
      else if (serviceType === 'hotel') fetchHotels();
    } catch (error) {
  ...
  const handleRefresh = async (formData) => {
    try {
      ...
      // Strict data mapping (no custom keys needed, frontend matches backend schema)
      if (payload.price !== undefined) payload.price = Number(payload.price);
      if (payload.stops !== undefined) payload.stops = Number(payload.stops);
      ...
      const servicePath = getServiceType();
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      
      const res = await axios.put(`http://localhost:5000/api/admin/service/${servicePath}/${formData._id}`, payload, config);
      
      console.log("API response:", res.data);
      toast.success(`${servicePath.charAt(0).toUpperCase() + servicePath.slice(1)} updated successfully`);
      
      // Update local state without fetching
      if (type === 'flights') {
        setFlights(flights.map(f => f._id === formData._id ? { ...f, ...formData } : f));
      } else if (type === 'trains') {
        setTrains(trains.map(t => t._id === formData._id ? { ...t, ...formData } : t));
      } else if (type === 'cabs') {
        setCabs(cabs.map(c => c._id === formData._id ? { ...c, ...formData } : c));
      }
  ...
  // Get service type from type param (plural to singular)
  const getServiceType = () => {
    const map = { flights: 'flight', trains: 'train', cabs: 'cab', hotels: 'hotel', packages: 'package' };
          ...
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-12 relative flex items-center justify-center">
              ...
              {isAdmin && (
                <div className="absolute right-0">
                  <AdminAddButton label="Add Package" onClick={() => { setEditingItem(null); setShowModal(true); }} />
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {domesticPackages.map((pkg) => (
                <div
                  key={pkg.id}
                  className="group relative bg-white rounded-2xl shadow-lg hover:shadow-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 border border-blue-100 cursor-pointer"
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); handlePackageBook(pkg); }}
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        {/* Horizontal Filter Bar - Hide for flights to use Sidebar instead */}
        {type !== 'flights' && type !== 'trains' && type !== 'cabs' && (
          <FilterBar
        ...
        {/* Sort Bar - Handled inside FlightList for flights now */}
        {type !== 'flights' && type === 'some_other_type' && (
          <SortBar
            flights={filteredFlights}
          ...
          <div className="flex items-center gap-2">
              ...
              <>
                ...
                <div>
                  <h1 className="text-lg font-bold text-gray-900 flex items-center gap-3">
                    ...
                    {isAdmin && (type === 'flights' || type === 'trains' || type === 'cabs') && (
                      <AdminAddButton 
                        label={`Add ${type.substring(0, type.length - 1)}`} 
            ...
            {type === 'cabs' && isAdmin && (
              <div className="w-full flex items-center justify-between">
                <h1 className="text-lg font-bold text-gray-900 flex items-center gap-3">
                  cabs
                  <AdminAddButton label="Add Cab" onClick={() => { setEditingItem(null); setShowModal(true); }} />
                </h1>
              </div>
            )}
            ...
            <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div className="hidden lg:block w-full lg:w-1/4 flex-shrink-0">
                <Filters
                  ...
                  availableAirlines={['Indigo', 'Air India', 'SpiceJet', 'Vistara', 'Go First', 'Akasa Air', 'AirAsia']}
                  serviceType="flights"
                />
              </div>
              ...
              {loading ? (
                ...
                <div className="space-y-3">
                  {sortedTrains.map((train) => (
                    <div
                      key={train._id}
                      className="bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-all p-4 flex flex-col lg:flex-row items-start lg:items-center gap-4 relative group"
                    >
                      {/* Admin badge */}
                      {isAdmin && (
                      ...
                      {isAdmin && (
                        <div className="absolute top-2 right-16 z-10 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button
                            onClick={() => handleEdit(train, 'train')}
                          ...
                          <button
                            onClick={() => handleDelete(train._id, 'train')}
                      ...
                      <div className="flex items-center gap-3 w-full lg:w-auto lg:min-w-[180px]">
                        ...
                        <div className="min-w-0">
                          <div className="text-sm font-semibold text-gray-900 truncate">{train.trainName || train.name}</div>
              ...
              {sortedCabs.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {sortedCabs.map((cab, idx) => (
                    <div
                      key={cab._id || cab.id}
                      className="group bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden relative"
                    >
                      {/* Admin badge */}
                      {isAdmin && (
                      ...
                      {isAdmin && (
                        <div className="absolute top-3 right-16 z-10 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <button
                            onClick={() => handleDelete(cab._id, 'cab')}
